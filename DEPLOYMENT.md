...
**Script 2**: `supabase/COMPLETE-FIX-all-tables.sql` (entire file)
- Fixes RLS policies for anonymous access

**Script 3**: `supabase/set-admin-role.sql` (MODIFY FIRST!)
```sql
...
**Script 4-7** (Optional sample data):
- `sample-committees.sql`
- `sample-secretariat.sql`
- `sample-schedule.sql`
- `sample-applications.sql`
...
- [ ] Add `metadata` export to `app/layout.tsx`:
  ...
  export const metadata = {
    ...
    description: 'Forging Destiny Among the Stars - Join us for an elite MUN experience',
    openGraph: {
      images: ['/og-image.png'],
    },
  }
...
**Fix:** Re-run `COMPLETE-FIX-all-tables.sql`
